...
export const generateMaletaPDF = (maleta: Maleta) => {
  ...
  doc.text(maleta.representative_name, 55, 55);
  ...
  doc.setFont('helvetica', 'bold');
  doc.text(`Valor total da maleta: R$ ${parseFloat(maleta.total_value).toFixed(2)}`, 20, finalY);
  doc.text(`Quantidade total de produtos: ${maleta.items.reduce((sum, item) => sum + item.quantity, 0)}`, 20, finalY + 10);
  
  // Commission info
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text('Comissão de vendas', 20, finalY + 30);
  doc.setFont('helvetica', 'normal');
  
  if (maleta.commission_settings.use_global) {
    maleta.commission_settings.tiers.forEach((tier, index) => {
      const yPos = finalY + 40 + (index * 8);
      doc.text(
    ...
    doc.text(`Comissão personalizada: ${maleta.commission_percentage}%`, 20, finalY + 40);
  }
  ...
  doc.text(maleta.representative_name, 50, pageHeight - 5, { align: 'center' });
